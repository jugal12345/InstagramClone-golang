...
import (
	...
	"github.com/jugal12345/go-lang/models"
...
func ExtraUserController(s *mgo.Session) *UserCont {
	return &UserCont{s}
}
...
func (pc PostController) Get_user_post(w http.ResponseWriter,r *http.Request, p httprouter.Params) {
	...
	err := pc.session.DB("insta-data").C("posts").Find(bson.M{"user":id}).Skip((page-1)*2).Limit(2).All(&posts)
	if err != nil {
		w.WriteHeader(404)
		return
	}
